...
//! to compile a set of Fortran files into a static archive for Cargo
...
use crate::utilities::{JoinOsStrs, OptionOsStrDisplay};
pub use tool::Tool;
...
impl Build {
    ...
    /// Specify the Fortran language standard version.
...
impl Build {
    ...
    fn ensure_check_file(&self) -> Result<PathBuf, Error> {
        ...
        let src = out_dir.join("flag_check.f90");
    ...
    fn find_working_gnu_prefix(&self, prefixes: &[&'static str]) -> Option<&'static str> {
        ...
        None.or_else(|| prefixes.first().copied())
    }
...
/// Returns true if `cc` has been disabled by `FC_FORCE_DISABLE`.
fn is_disabled() -> bool {
    ...
    fn compute_is_disabled() -> bool {
        match std::env::var_os("FC_FORCE_DISABLE") {
            ...
            None => false,
            // Respect `FC_FORCE_DISABLE=0` and some simple synonyms, otherwise
            // we're disabled. This intentionally includes `FC_FORCE_DISABLE=""`
